...
- Slice 1 red/green verification:
  ...
  - Slice 8 QA/release verification:
    ...
    - `git diff --check` passed.
  - Staging invite-link operational repair verification:
    - `npx vercel inspect https://staging.recetasfamilia.app` confirmed the clean staging alias is `dpl_C5keEnFSnsMmxKSTDgsWuUJKziEM` / `https://recetas-69hle56xn-luisfleitas-1188s-projects.vercel.app`.
    - `npx vercel curl /api/health --deployment https://staging.recetasfamilia.app -- --silent --show-error --include` returned HTTP 200 with healthy app, database, and Blob checks.
    - `npx vercel curl /api/admin/repair-family-invite-migration --deployment https://staging.recetasfamilia.app -- --silent --show-error --request POST --write-out 'HTTP_STATUS:%{http_code}\n'` returned HTTP 404 after the clean redeploy, confirming the temporary repair route is no longer present.
    - Authenticated staging smoke registered a test user, created family id `2`, and `POST /api/families/2/invite-links` returned HTTP 201 with invite id `4`, `inviteType: "link"`, `usageType: "single_use"`, and `state: "active"`.
