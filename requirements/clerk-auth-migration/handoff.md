...
- Status: Tasks 1, 2, 3, and 4 complete; Task 5 is in progress. A staging logout click bugfix is deployed from `codex/feature/fix-staging-logout` to `https://staging.recetasfamilia.app`; a follow-up fix on `codex/fix/logout-hard-navigation` now forces a document navigation to the public landing page after successful logout so the signed-out state appears without pressing F5. PR #37 is open back to `pre-main`, and `https://staging.recetasfamilia.app` is aliased to the fixed preview deployment.
...
- Staging logout click bugfix updates the shared `LogoutButton` to validate `/api/auth/logout`, visibly redirect after logout, and show a localized error if logout fails.
- Follow-up local fix changes the shared `LogoutButton` redirect from App Router soft navigation to `window.location.assign("/")`, because the logout POST succeeds but the client shell can keep showing stale signed-in state until a document reload. The intended signed-out destination is the public landing page, not `/login`.
...
- Logout hard-navigation follow-up Preview deployed from `codex/fix/logout-hard-navigation`: `dpl_EncDNbUeuwqkNUKQSeCX18faENeC` / `https://recetas-ktx36bxhs-luisfleitas-1188s-projects.vercel.app`, aliased to `https://staging.recetasfamilia.app`.
...
- Merge PR #37 after review/checks, then verify interactively on `https://staging.recetasfamilia.app` that clicking logout lands on the public home page as an unauthenticated user without pressing F5.
...
- The shared logout UI must keep performing a document navigation after successful provider-owned logout; App Router soft navigation can leave the client shell showing stale signed-in state until F5 even when the POST path succeeds.
...
- `node --experimental-strip-types --loader ./scripts/alias-loader.mjs --test scripts/logout-button-client.test.ts` failed before the follow-up fix because `LogoutButton` still used `router.replace("/login")`.
- `node --experimental-strip-types --loader ./scripts/alias-loader.mjs --test scripts/logout-button-client.test.ts` passed after switching the shared `LogoutButton` to `window.location.assign("/login")`.
- `node --experimental-strip-types --loader ./scripts/alias-loader.mjs --test scripts/logout-button-client.test.ts` failed before the landing-page destination update because `LogoutButton` still used `window.location.assign("/login")`.
- `node --experimental-strip-types --loader ./scripts/alias-loader.mjs --test scripts/logout-button-client.test.ts` passed after switching the shared `LogoutButton` destination to `window.location.assign("/")`.
- `npm run test:auth` passed after the follow-up logout hard-navigation fix.
- `npm run build` passed after the follow-up logout hard-navigation fix.
- `git diff --check` passed after the follow-up logout hard-navigation fix.
...
- `npx vercel curl /api/health --deployment https://staging.recetasfamilia.app` returned healthy app and database checks after the alias update.
- `npm run test:auth`, `npm run build`, and `git diff --check` passed after changing the logout hard-navigation destination from `/login` to `/`.
...
- `npx vercel curl /api/health --deployment https://staging.recetasfamilia.app` returned healthy app and database checks after the landing-page alias update.
