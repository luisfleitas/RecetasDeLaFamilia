# Phase 2 Runnable Checklist

Use this checklist to validate Phase 2 recipe sharing behavior (`private` vs `family` visibility with family-link access control).

## Prerequisites
- App is running locally (`BASE_URL`, default `http://localhost:3000`).
- Migrations are applied (`npx prisma migrate deploy`).
- Seed users exist (`alice`, `bob`) with password `Password123!` (or set env vars below).
