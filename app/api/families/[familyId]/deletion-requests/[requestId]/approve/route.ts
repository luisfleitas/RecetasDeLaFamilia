  ...
  try {
    ...
    if (isPhase3Enabled()) {
      await recordAuditEvent(prisma, {
        eventType: "family_deletion_request_vote_approved",
        requestId: requestIdHeader,
        actorUserId: authUser.userId,
        familyId: result.familyDeleted ? null : familyId,
      });
      await recordMetric(prisma, {
        metricName: "family_deletion_request_vote_approved",
        requestId: requestIdHeader,
        actorUserId: authUser.userId,
        familyId: result.familyDeleted ? null : familyId,
    ...
    const message = error instanceof Error ? error.message : "Unexpected error while approving deletion request";
