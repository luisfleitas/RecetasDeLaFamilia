...
import { cancelDeletionRequest } from "@/lib/families/deletion-actions";
  ...
  try {
    ...
    if (isPhase3Enabled()) {
      await recordAuditEvent(prisma, {
        eventType: "family_deletion_request_cancelled",
        requestId: requestIdHeader,
        actorUserId: authUser.userId,
        familyId,
      });
      await recordMetric(prisma, {
        metricName: "family_deletion_request_cancelled",
        requestId: requestIdHeader,
        actorUserId: authUser.userId,
        familyId,
    ...
    const message = error instanceof Error ? error.message : "Unexpected error while cancelling deletion request";
    return withRequestId(NextResponse.json({ error: message }, { status: 500 }), requestIdHeader);
  }
}
